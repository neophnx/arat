...
class Annotation(object):
    ...
    def _parse_type(s):
        ...
        if s[0].isalnum() or s[0] == '*':
            ...
            if not m:
                raise SyntaxError(
                    s, "Failed to parse type in \"%s\"" % fields[1])
            return m.group(1)
...
def argparser():
    ...
    ap.add_argument("-a1", "--a1types", default="Protein",
                    metavar="TYPE[,TYPE...]", help="Annotation types to place into .a1 file")
    ap.add_argument("-a2", "--a2types",
                    default="[OTHER]", metavar="TYPE[,TYPE...]", help="Annotation types to place into .a2 file")
    ...
    ap.add_argument("-s", "--skipempty", default=False,
                    action="store_true", help="Skip output for empty split files")
    ap.add_argument("-i", "--idrewrite", default=False, action="store_true",
...
def split_annotations(annotations, typemap):
    ...
    for a in annotations:
        if a.type in typemap:
            ...
            raise ArgumentError(
                "Don't know where to place annotation of type '%s'" % a.type)
        s = typemap[t]
...
def type_mapping(arg):
    ...
    for suff, typestr in (("a1", arg.a1types),
        ...
        for ts in typestr.split(","):
            ...
            if t in m:
                raise ArgumentError(
                    "Split for '%s' ambiguous (%s or %s); check arguments." % (ts, m[t], suff))
            m[t] = suff
    return m
...
def verify_split(origlines, splitlines):
    ...
    for l in split_only:
        print("Split error: split contains extra line '%s'" %
              l, file=sys.stderr)
...
def main(argv=None):
    ...
    for fn in arg.files:
        try:
            ...
            print("Fatal: unexpected error processing %s" %
                  fn, file=sys.stderr)
            raise
