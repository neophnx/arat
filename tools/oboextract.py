...
class Term:
    ...
    def resolve_references(self, term_by_id, term_by_name=None):
        ...
        for ptid, pname in self.is_a:
            if ptid not in term_by_id:
                print("Warning: is_a term '%s' not found, ignoring" %
                      ptid, file=sys.stderr)
                continue
            ...
            if self in parent.children:
                print("Warning: dup is-a parent %s for %s, ignoring" %
                      (ptid, str(self)), file=sys.stderr)
            else:
        ...
        for prel, ptid, pname in self.part_of:
            if ptid not in term_by_id:
                print("Error: part_of term '%s' not found, ignoring" %
                      ptid, file=sys.stderr)
                continue
            ...
            if self in pobject.components:
                print("Warning: dup part-of parent %s for %s, ignoring" %
                      (ptid, str(self)), file=sys.stderr)
            else:
    ...
    def _case_normalize(self, cn_func):
        ...
        for i in range(len(self.synonyms)):
            self.synonyms[i] = (
                cn_func(self.synonyms[i][0]), self.synonyms[i][1])
...
def parse_obo(f, limit_prefixes=None, include_nameless=False):
    ...
    skip_block = True
    tid, prefix, name, synonyms, definitions, comments, is_a, part_of, obsolete = None, None, None, [
    ], [], [], [], [], False
    for ln, l in enumerate(f):
        ...
        if l.strip() == "[Typedef]":
            ...
            m = re.match(
                r'^id: (([A-Za-z](?:\S*(?=:)|[A-Za-z_]*)):?\S+)\s*$', l)
            if m is None:
                print("line %d: failed to match id, ignoring: %s" %
                      (ln, l.rstrip()), file=sys.stderr)
            ...
            if m:
                ...
                if m is not None:
                    ...
                    print("Error: failed to parse '%s'; ignoring is_a" %
                          l, file=sys.stderr)
            ...
            m = re.match(
                r'^relationship: +(?:OBO_REL:)?(\S+) +(\S+) *(?:\{[^{}]*\} *)?\! *(.*?)\s*$', l)
            if m:
                ...
                if m is not None:
                    ...
                    print("Error: failed to parse '%s'; ignoring part_of" %
                          l, file=sys.stderr)
            ...
            m = re.match(
                r'^synonym: "(.*)" ([A-Za-z_ -]*?) *\[.*\]\s*(?:\{.*\}\s*)?$', l)
            assert m is not None, "Error: failed to parse '%s'" % l
            synstr, syntype = m.groups()
            if synstr == "":
                print("Note: ignoring empty synonym on line %d: %s" %
                      (ln, l.strip()), file=sys.stderr)
            else:
            ...
            if definition == "":
                print("Note: ignoring empty def on line %d: %s" %
                      (ln, l.strip()), file=sys.stderr)
            else:
            ...
            if (obsolete or
                ...
                if name is None and not include_nameless:
                    print("Note: ignoring term without name (%s) on line %d" %
                          (tid, ln), file=sys.stderr)
                else:
                    if tid not in term_by_id:
                        ...
                        print(
                            "Error: duplicate ID '%s'; discarding all but first definition" % tid, file=sys.stderr)
                tid, prefix, name, synonyms, definitions, comments, is_a, part_of, obsolete = None, None, None, [
                ], [], [], [], [], False
            else:
                pass
...
def argparser():
    ...
    ap = argparse.ArgumentParser(
        description="Extract terms from OBO ontology.")
    ap.add_argument("-l", "--limit", default=None, metavar="PREFIX",
                    help="Limit processing to given ontology prefix or prefixes (multiple separated by \"|\").")
    ap.add_argument("-c", "--comments", default=False,
                    action="store_true", help="Include comments.")
    ap.add_argument("-d", "--depth", default=None, metavar="INT",
                    help="Limit extraction to given depth from initial nodes.")
    ap.add_argument("-nc", "--no-case-normalization", default=False, action="store_true",
                    help="Skip heuristic case normalization of ontology terms.")
    ap.add_argument("-nm", "--no-multiple-inheritance", default=False,
                    action="store_true", help="Exclude subtrees involving multiple inheritance.")
    ap.add_argument("-ns", "--no-synonyms", default=False,
                    action="store_true", help="Do not extract synonyms.")
    ap.add_argument("-nd", "--no-definitions", default=False,
                    action="store_true", help="Do not extract definitions.")
    ap.add_argument("-e", "--exclude", default=[], metavar="TERM",
                    nargs="+", help="Exclude subtrees rooted at given TERMs.")
    ap.add_argument("-s", "--separate-children", default=False, action="store_true",
                    help="Separate subontologies found as children of the given term.")
    ap.add_argument("file", metavar="OBO-FILE", help="Source ontology.")
    ap.add_argument("-p", "--separate-parents", default=False, action="store_true",
                    help="Separate subontologies of parents of the given terms.")
    ap.add_argument("terms", default=[], metavar="TERM",
                    nargs="*", help="Root terms from which to extract.")
    return ap
...
def get_subtree_terms(root, collection=None, depth=0):
    ...
    if options.no_multiple_inheritance and len(root.parents) > 1:
        ...
        if multiple_parent_skip_count < 10:
            print("Note: not traversing subtree at %s %s: %d parents" %
                  (root.tid, root.name, len(root.parents)), file=sys.stderr)
        elif multiple_parent_skip_count == 10:
            print(
                "(further 'not traversing subtree; multiple parents' notes suppressed)", file=sys.stderr)
...
def main(argv=None):
    ...
    f = open(fn)
    if limit_prefixes:
        print('None: experimental: applying limit_prefixes only in output',
              file=sys.stderr)
    ...
    print("OK, parsed %d (non-obsolete) terms." %
          len(all_terms), file=sys.stderr)
    ...
    for t in all_terms:
        if t.name not in term_by_name:
            ...
            print("Warning: duplicate name '%s'; no name->ID mapping possible" %
    ...
    for t in all_terms:
        for ptid, pname in t.is_a:
            if ptid not in term_by_id:
                print("Error: is_a term '%s' not found, removing" %
                      ptid, file=sys.stderr)
                continue
            parent = term_by_id[ptid]
            # name is not required information; check if included
            ...
            if pname is not None and pname in term_by_name and term_by_name[pname] is not None:
                if parent != term_by_name[pname]:
                    print("Warning: given parent name '%s' mismatches parent term name (via ID) '%s'" % (
                        parent.name, pname), file=sys.stderr)
            if t in parent.children:
                print("Warning: ignoring dup parent %s for %s" %
                      (ptid, str(t)), file=sys.stderr)
            else:
    ...
    if not arg.separate_parents:
        ...
        if len(rootterms) == 0:
            ...
            print("Extracting from %d root terms." %
                  len(rootterms), file=sys.stderr)
        ...
        if _PYTHON3:
            rootterms.sort(key=lambda a: a.name) # pylint: disable=undefined-variable
        else:
            rootterms.sort(lambda a, b: cmp(a.name, b.name)) # pylint: disable=undefined-variable
        arg.separate_children = True

        # debugging
        print("Splitting at the following:",
              ",".join(rootterms), file=sys.stderr)
